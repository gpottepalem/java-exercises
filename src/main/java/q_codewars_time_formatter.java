...
class Scratch {
    ...
    static int computeAndAddDuration(int seconds, Duration duration, List<String> durationsList) {
        int divisor = switch(duration) {
        ...
        int numberOfDurations = seconds / divisor;
        if (numberOfDurations > 0 ) {
            durationsList.add(String.format("%d %s%s", numberOfDurations, duration.name().toLowerCase(), numberOfDurations > 1 ? "s" : ""));
            seconds = seconds - (numberOfDurations * divisor);
        }
    ...
    public static String formatDuration(int seconds) {
        ...
        seconds = computeAndAddDuration(seconds, Duration.YEAR, durations);
        seconds = computeAndAddDuration(seconds, Duration.DAY, durations);
        seconds = computeAndAddDuration(seconds, Duration.HOUR,  durations);
        seconds = computeAndAddDuration(seconds, Duration.MINUTE,  durations);
        computeAndAddDuration(seconds, Duration.SECOND, durations);

        return switch (durations.size()) {
            case 1 -> durations.get(0);
            default ->
                String.format(
                    "%s and %s",
                    durations.stream()
                        .limit(durations.size() - 1)
